        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button className="rounded-full bg-white text-slate-900 hover:bg-slate-100 h-10 px-6 gap-2 font-black text-[10px] uppercase shadow-xl transition-all active:scale-95">
            ...
            <DropdownMenuContent align="center" className="w-56 mb-4 rounded-[1.5rem] shadow-2xl p-2 border-slate-100">
              <DropdownMenuItem className="rounded-xl h-11 text-xs font-bold" onClick={() => onAction('view')}>
                <Zap className="mr-2 h-4 w-4" /> {t('view')}
              </DropdownMenuItem>
              <DropdownMenuItem className="rounded-xl h-11 text-xs font-bold" onClick={() => onAction('print')}>
                <Printer className="mr-2 h-4 w-4" /> {t('printSelected')}
              </DropdownMenuItem>
              <DropdownMenuItem className="rounded-xl h-11 text-xs font-bold" onClick={() => onAction('download')}>
                <Download className="mr-2 h-4 w-4" /> {t('downloadSelected')}
              </DropdownMenuItem>
              <DropdownMenuSeparator className="my-1 bg-slate-50" />
              <DropdownMenuItem className="rounded-xl h-11 text-xs font-bold" onClick={() => onAction('status')}>
                <CheckCircle2 className="mr-2 h-4 w-4" /> {t('changeStatus')}
              </DropdownMenuItem>
              <DropdownMenuItem className="rounded-xl h-11 text-xs font-bold" onClick={() => onAction('tags')}>
                <Tag className="mr-2 h-4 w-4" /> {t('addTags')}
              </DropdownMenuItem>
              <DropdownMenuItem className="rounded-xl h-11 text-xs font-bold" onClick={() => onAction('duplicate')}>
                <Copy className="mr-2 h-4 w-4" /> {t('duplicateSelected')}
              </DropdownMenuItem>
              <DropdownMenuSeparator className="my-1 bg-slate-50" />
              <DropdownMenuItem className="rounded-xl h-11 text-xs font-bold text-red-600 focus:bg-red-50 focus:text-red-700" onClick={() => onAction('delete')}>
                <Trash2 className="mr-2 h-4 w-4" /> {t('deleteSelected')}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>

          <Button variant="ghost" size="icon" onClick={onClear} className="h-10 w-10 rounded-full text-white/60 hover:text-white hover:bg-white/10 transition-colors">
            <X className="h-5 w-5" />
          </Button>
        </div>
      </div>
