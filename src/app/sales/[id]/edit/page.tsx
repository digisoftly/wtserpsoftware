...
export default function EditInvoicePage() {
  ...
  React.useEffect(() => {
    if (invoice) {
      ...
      setManualCustomerName(invoice.customerId === "manual" ? invoice.customerName : "");
              ...
              {isManualCustomer ? (
                <>
                  <div className="space-y-1.5 lg:col-span-2">
                    ...
                    <Input className="h-11 rounded-xl bg-slate-50 border-none ring-1 ring-slate-200 text-xs font-bold" value={manualCustomerName} onChange={e => setManualCustomerName(e.target.value)} />
                  </div>
                </>
              ) : (
                <>
          ...
          <Card className="p-6 rounded-[2rem] border-none shadow-sm ring-1 ring-slate-100 bg-white space-y-6">
            <div className="flex gap-2">
              <Select onValueChange={handleAddProduct}>
                <SelectTrigger className="h-12 flex-1 rounded-2xl bg-slate-50 border-none ring-1 ring-slate-100 shadow-sm font-bold text-xs">
                ...
                <TableBody>
                  {lineItems.map((item, idx) => (
                    <TableRow key={idx} className="group hover:bg-slate-50/50 transition-colors h-20">
                      ...
                      <TableCell>
                        <Input type="number" className="h-9 text-center font-black text-sm rounded-xl w-16 bg-slate-50 border-none mx-auto" value={item.qty} disabled={item.isSerialized} onChange={e => handleUpdateItem(idx, 'qty', e.target.value)} />
                      </TableCell>
                      ...
                      <TableCell><Button variant="ghost" size="icon" className="h-9 w-9 text-red-500 rounded-full" onClick={() => setLineItems(lineItems.filter((_, i) => i !== idx))}><X className="h-4 w-4" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
