...
import { collection, serverTimestamp, query, orderBy, doc, increment, runTransaction, where } from "firebase/firestore"
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button className="bg-green-600 hover:bg-green-700 gap-2 rounded-full px-8 shadow-lg h-12 font-bold" onClick={() => setIsAddModalOpen(true)}>
          <ShoppingCart className="h-5 w-5" /> New POS Transaction
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard title="Outstandings" value={`৳${invoices?.reduce((s, i) => s + (i.totalAmount || 0), 0).toLocaleString()}`} icon={Clock} colorClass="bg-orange-500" />
              ...
              <TableBody>
                {invoices?.map((inv) => (
                  <TableRow key={inv.id} className="hover:bg-muted/20">
                    ...
                    <TableCell>{customers?.find(c => c.id === inv.customerId)?.firstName || "Guest"}</TableCell>
      ...
      <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
        <DialogContent className="max-w-6xl w-[95vw] max-h-[95vh] overflow-y-auto p-0 border-none shadow-2xl">
          <DialogHeader className="p-6 bg-green-600 text-white">
            <DialogTitle className="text-2xl font-headline flex items-center gap-3"><ShoppingCart className="h-6 w-6" /> Dynamic POS Terminal</DialogTitle>
          </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input placeholder="Ready to scan..." className="pl-9 h-12 border-2 border-green-50 rounded-xl focus:border-green-500" value={scanTerm} onChange={e => handleScan(e.target.value)} />
                  </div>
                </div>
              </div>
                      ...
                      {lineItems.length === 0 ? (
                        ...
                        lineItems.map((item, idx) => (
                          <TableRow key={idx}>
                            ...
                            <TableCell>
                              <Input type="number" disabled={item.requiresSerial} value={item.quantity} className="h-8 text-xs font-bold" onChange={e => {
                            ...
                            <TableCell><Button variant="ghost" size="icon" className="h-8 w-8 text-red-500" onClick={() => setLineItems(lineItems.filter((_, i) => i !== idx))}><Trash2 className="h-4 w-4" /></Button></TableCell>
                          </TableRow>
                        ))
                      )}
                    </TableBody>
