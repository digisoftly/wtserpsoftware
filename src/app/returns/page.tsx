  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-xl font-bold font-headline text-red-600">{t('returns')}</h1>
        <Button 
          variant="destructive" 
      ...
      <Tabs defaultValue="sales" onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-white border p-1 rounded-xl shadow-sm mb-6 flex h-10 ring-1 ring-slate-100">
          <TabsTrigger value="sales" className="rounded-lg gap-2 flex-1 text-[10px] uppercase font-black h-8">
            <ShoppingCart className="h-3.5 w-3.5" /> {t('sales')}
          </TabsTrigger>
          <TabsTrigger value="purchase" className="rounded-lg gap-2 flex-1 text-[10px] uppercase font-black h-8">
        ...
        <TabsContent value="sales" className="space-y-4">
          <div className="flex flex-col sm:flex-row items-center gap-4 bg-white p-3 rounded-xl border shadow-sm ring-1 ring-slate-50">
            <div className="relative flex-1 w-full max-w-sm">
              ...
              <Input placeholder={t('search')} className="pl-9 h-9 border-none bg-background text-xs ring-1 ring-slate-200" />
            </div>
          </div>
                ...
                <Table>
                  <TableHeader className="bg-muted/20">
                    <TableRow>
                      ...
                      <TableHead className="text-[10px] uppercase font-bold h-9">{t('invoiceNumber')}</TableHead>
                      <TableHead className="text-[10px] uppercase font-bold h-9">{t('reason')}</TableHead>
                  ...
                  <TableBody>
                    {salesReturns?.map((r) => (
                      <TableRow key={r.id} className="h-12 hover:bg-muted/10 transition-colors">
                        <TableCell className="font-black text-[10px] uppercase">#SR-{r.id.slice(-4)}</TableCell>
                        <TableCell className="text-xs font-bold text-red-600">
                          {invoices?.find(inv => inv.id === r.salesInvoiceId)?.invoiceNumber || "---"}
                        </TableCell>
                        <TableCell className="text-xs font-bold text-muted-foreground">{r.reason}</TableCell>
                ...
                <Table>
                  ...
                  <TableBody>
                    {purchaseReturns?.map((r) => (
                      <TableRow key={r.id} className="h-12 hover:bg-muted/10 transition-colors">
                        <TableCell className="font-black text-[10px] uppercase">#PR-{r.id.slice(-4)}</TableCell>
                        <TableCell className="text-xs font-bold text-red-600">
      ...
      <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
        <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl">
          <DialogHeader className="bg-red-600 p-6 text-white flex-row items-center gap-3">
            ...
            <DialogTitle className="text-xl font-bold font-headline uppercase">{t('addReturn')}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleAddReturn} className="p-6 space-y-4 bg-slate-50">
            <div className="space-y-1">
              <Label className="text-[10px] font-bold uppercase text-muted-foreground">{activeTab === "sales" ? t('invoiceNumber') : t('poNumber')}</Label>
              <Select name={activeTab === "sales" ? "invoiceId" : "poId"} required>
                ...
                <SelectContent>
                  {activeTab === "sales" 
                    ? invoices?.map(inv => <SelectItem key={inv.id} value={inv.id} className="text-xs font-bold">{inv.invoiceNumber} (৳{inv.totalAmount})</SelectItem>)
                    : purchaseOrders?.map(po => <SelectItem key={po.id} value={po.id} className="text-xs font-bold">{po.orderNumber} (৳{po.totalAmount})</SelectItem>)
                  }
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-[10px] font-bold uppercase text-muted-foreground">{t('returnAmount')} (৳)</Label>
              <Input name="amount" type="number" step="0.01" required placeholder="0.00" className="h-11 rounded-xl text-xs ring-1 ring-slate-200 border-none font-bold" />
            </div>

            <div className="space-y-1">
              <Label className="text-[10px] font-bold uppercase text-muted-foreground">{t('reason')}</Label>
              <Input name="reason" required placeholder={t('reason')} className="h-11 rounded-xl text-xs ring-1 ring-slate-200 border-none" />
            </div>

            <Button type="submit" className="w-full bg-red-600 hover:bg-red-700 h-12 rounded-2xl text-[10px] font-black uppercase tracking-widest mt-4 shadow-xl shadow-red-100 active:scale-95 transition-all">{t('save')}</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
