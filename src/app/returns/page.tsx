  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold font-headline text-red-600 flex items-center gap-2">
            <RotateCcw className="h-8 w-8" />
            Returns & RMA
          </h1>
          <p className="text-muted-foreground mt-1">Process customer and supplier credit notes</p>
        </div>
        <Button 
          variant="destructive" 
          className="gap-2 rounded-full shadow-lg shadow-red-100" 
      ...
      <Tabs defaultValue="sales" onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-white border p-1 rounded-xl shadow-sm mb-6">
          <TabsTrigger value="sales" className="rounded-lg gap-2">
            <ShoppingCart className="h-4 w-4" /> Sales Returns
          </TabsTrigger>
          <TabsTrigger value="purchase" className="rounded-lg gap-2">
            <Package className="h-4 w-4" /> Purchase Returns
          </TabsTrigger>
        </TabsList>
                ...
                <TableBody>
                  {salesReturns?.map((r) => (
                    <TableRow key={r.id} className="hover:bg-muted/30 transition-colors">
                      <TableCell className="font-bold text-xs">#S-RET-{r.id.slice(-6).toUpperCase()}</TableCell>
                      ...
                      <TableCell className="font-bold text-red-600">-${r.returnAmount?.toLocaleString()}</TableCell>
                      <TableCell><Badge variant="outline" className="border-red-200 text-red-700 bg-red-50 capitalize">{r.status}</Badge></TableCell>
              ...
              <Table>
                ...
                <TableBody>
                  {purchaseReturns?.map((r) => (
                    <TableRow key={r.id} className="hover:bg-muted/30 transition-colors">
                      <TableCell className="font-bold text-xs">#P-RET-{r.id.slice(-6).toUpperCase()}</TableCell>
                      <TableCell className="text-xs">
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleAddReturn} className="space-y-4 pt-4">
            <div className="space-y-2">
              <Label>{activeTab === "sales" ? "Select Original Invoice" : "Select Original PO"}</Label>
            ...
            <div className="space-y-2">
              ...
              <Input name="reason" required placeholder={activeTab === "sales" ? "e.g. Damaged product" : "e.g. Specification mismatch"} />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button type="submit" className="bg-red-600 hover:bg-red-700">Submit Return</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
...
function EmptyState({ tab, onAdd }: { tab: string, onAdd: () => void }) {
  return (
    <div className="p-16 bg-white rounded-2xl border-2 border-dashed flex flex-col items-center justify-center text-center">
      ...
      <h2 className="text-2xl font-headline font-bold">No {tab === "sales" ? "Sales" : "Purchase"} Returns</h2>
      <p className="text-muted-foreground max-w-sm mt-2 text-lg">
        {tab === "sales" 
          ? "Keep track of customer returns, RMA requests, and refund statuses in this section."
          : "Manage outgoing returns to suppliers and track pending credit from vendors."
        }
      </p>
      <Button variant="destructive" className="mt-8 px-8 py-6 text-lg rounded-full" onClick={onAdd}>
        Process First {tab === "sales" ? "Sales" : "Purchase"} Return
      </Button>
    </div>
  )
}
