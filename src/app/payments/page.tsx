...
export default function GlobalPaymentsPage() {
  ...
  const filteredPayments = React.useMemo(() => {
    return payments?.filter(p => 
      ...
      p.reference?.toLowerCase().includes(deferredSearch.toLowerCase())
    );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ title: t('success'), description: "Payment reversed successfully." });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="text-xl font-bold text-slate-900 uppercase tracking-tight">{t('payment')}</h1>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="h-9 rounded-lg gap-2 px-6 font-black text-[11px] uppercase tracking-widest bg-emerald-600 hover:bg-emerald-700" asChild>
            <Link href="/sales">
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex flex-col sm:flex-row gap-3 items-center justify-between ring-1 ring-slate-100">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input 
            ...
            className="pl-10 h-10 w-full rounded-lg bg-slate-50/50 border-none text-xs font-bold focus:ring-1 focus:ring-emerald-600 outline-none transition-all" 
            value={searchTerm} 
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="text-[10px] uppercase font-black text-slate-500 h-12 text-right pr-6">Amount Received</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="h-40 text-center text-slate-300 text-[10px] font-black uppercase tracking-[0.2em] italic">No records found</TableCell></TableRow>
              ) : (
                filteredPayments?.map((p) => (
                  <TableRow key={p.id} className={cn("h-20 hover:bg-muted/5 transition-colors group border-slate-50", selectedIds.includes(p.id) && "bg-blue-50/30")}>
                    ...
                    <TableCell className="pl-4">
                       <span className="text-xs font-black text-emerald-600 font-mono uppercase">{p.receiptNumber}</span>
                    ...
                    <TableCell className="text-center">
                      <span className="text-[9px] font-black bg-slate-100 text-slate-600 px-2 py-0.5 rounded uppercase">{p.paymentMethod}</span>
                    </TableCell>
                      ...
                      <div className="flex justify-end gap-1">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-56 rounded-2xl shadow-2xl p-2 border-slate-100">
                            ...
                            <DropdownMenuItem onClick={() => router.push(`/sales/payments/history/${p.invoiceId}`)} className="rounded-xl h-10 text-xs font-bold py-1.5"><History className="mr-2 h-4 w-4" /> Invoice History</DropdownMenuItem>
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent className="rounded-[2.5rem] border-none p-10 shadow-2xl max-w-sm">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs font-bold text-slate-500">This will subtract the amount from the invoice's collection total and remove the entry from accounts.</AlertDialogDescription>
          </AlertDialogHeader>
