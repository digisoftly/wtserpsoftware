...
import { Truck, Plus, Search, Loader2, MoreVertical, Mail, Phone, MapPin, Filter, Edit, Trash2, Eye, Download, ShieldCheck, ShoppingCart, AlertCircle } from "lucide-react"
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold font-headline text-amber-700">Suppliers</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <KPICard title="Total Suppliers" value={stats.total} icon={Truck} colorClass="bg-blue-600" subtext="Registered vendors" />
        <KPICard title="Payable Amount" value={`৳${stats.payable.toLocaleString()}`} icon={AlertCircle} colorClass="bg-red-600" subtext="Outstanding debt" />
      </div>

      <div className="flex gap-2">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
          <Input placeholder="Search vendor name..." className="pl-9 h-9 border-none bg-white shadow-sm ring-1 ring-slate-100 text-xs" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader className="bg-muted/20">
                <TableRow>
                  <TableHead className="text-[10px] uppercase font-bold h-9">Vendor Name</TableHead>
                  <TableHead className="text-[10px] uppercase font-bold h-9">Contact</TableHead>
                  <TableHead className="text-[10px] uppercase font-bold h-9">Payable</TableHead>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-32">
                          ...
                          <DropdownMenuItem className="text-red-600 text-xs" onClick={() => { setSelectedRecord(s); setIsDeleteAlertOpen(true); }}>
      ...
      <Dialog open={isAddModalOpen || isEditModalOpen} onOpenChange={(open) => { if(!open) { setIsAddModalOpen(false); setIsEditModalOpen(false); setSelectedRecord(null); } }}>
        <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl">
          <DialogHeader className="bg-amber-700 p-6 text-white flex-row items-center gap-3">
            ...
            <DialogTitle className="text-xl font-bold font-headline uppercase">{isEditModalOpen ? "Edit Supplier" : "New Supplier"}</DialogTitle>
          </DialogHeader>
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle className="font-headline">Delete Supplier?</AlertDialogTitle><AlertDialogDescription className="text-xs">Record will be permanently deleted.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel className="rounded-full text-[10px] uppercase font-bold h-9">Cancel</AlertDialogCancel><AlertDialogAction className="bg-red-600 rounded-full text-[10px] uppercase font-bold h-9" onClick={handleDeleteSupplier}>Delete</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  )
