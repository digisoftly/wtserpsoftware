...
import { Search, Loader2, Edit, Trash2, FolderTree, Save, Plus } from "lucide-react"
...
export default function MasterCategoriesPage() {
  ...
  return (
    <div className="space-y-6 pb-10">
      <h1 className="text-xl font-bold font-headline text-pink-600 uppercase">{t('categories')}</h1>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={c.id} className="h-12 hover:bg-muted/5 group">
                  ...
                  <TableCell className="text-right pr-6">
                    <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-pink-600" onClick={() => setSelectedRecord(c)}><Edit className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600" onClick={() => deleteDoc(doc(db!, "companies", companyId!, "master_categories", c.id))}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
