...
export default function MasterUnitsPage() {
  ...
  return (
    <div className="space-y-6 pb-10">
      <h1 className="text-xl font-bold font-headline text-blue-600 uppercase">{t('nav.master')} / {t('nav.units')}</h1>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="p-6 h-fit rounded-2xl border-none ring-1 ring-slate-100 shadow-sm bg-white">
          <h2 className="text-xs font-black uppercase text-slate-400 mb-6 tracking-widest">{selectedRecord ? t('common.edit') : t('common.add')}</h2>
          <form onSubmit={handleSave} className="space-y-4">
            <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase">{t('common.label')} *</Label><Input name="name" defaultValue={selectedRecord?.name} required className="h-10 text-xs rounded-xl" /></div>
            <div className="space-y-1.5"><Label className="text-[10px] font-bold uppercase">{t('forms.unit')} *</Label><Input name="shortName" defaultValue={selectedRecord?.shortName} required className="h-10 text-xs rounded-xl" /></div>
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-dashed">
              <Label className="text-xs font-bold">{t('common.default') || 'Default'}</Label>
            ...
            <div className="pt-4 flex gap-2">
              <Button type="submit" disabled={isSubmitting} className="flex-1 bg-blue-600 hover:bg-blue-700 h-11 rounded-xl text-[10px] font-black uppercase">
                {isSubmitting ? <Loader2 className="animate-spin h-4 w-4" /> : <Save className="h-4 w-4" />} {t('common.save')}
              </Button>
              {selectedRecord && <Button type="button" variant="ghost" onClick={() => setSelectedRecord(null)} className="rounded-xl h-11 px-4 text-[10px] font-black uppercase">{t('common.cancel')}</Button>}
            </div>
          </form>
        </Card>
        ...
        <Card className="lg:col-span-2 border-none shadow-sm rounded-2xl overflow-hidden bg-white ring-1 ring-slate-100">
          <div className="p-4 border-b bg-slate-50/50 flex items-center justify-between">
            <div className="relative w-64">
              ...
              <Input placeholder={t('common.search')} className="pl-9 h-9 border-none bg-white ring-1 ring-slate-100 text-xs" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{units?.length || 0} {t('nav.units')}</span>
          </div>
          <Table>
            <TableHeader className="bg-muted/10">
              <TableRow>
                <TableHead className="h-10 text-[10px] uppercase font-black pl-6">{t('common.label')}</TableHead>
                <TableHead className="h-10 text-[10px] uppercase font-black">{t('forms.unit')}</TableHead>
                ...
                <TableRow key={`${u.id}-${idx}`} className="h-12 hover:bg-muted/5 group">
                  ...
                  <TableCell className="text-right pr-6">
                    <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600" onClick={() => { if(confirm(t('common.confirmDelete'))) deleteDoc(doc(db!, "companies", companyId!, "master_units", u.id)) }}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
