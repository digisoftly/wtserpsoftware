...
import { Plus, Search, Loader2, MoreVertical, Edit, Trash2, Database, Ruler } from "lucide-react"
...
export default function MasterUnitsPage() {
  ...
  const handleBulkAction = async (action: string) => {
    ...
    if (action === 'delete') {
      if (confirm(`Delete ${selectedIds.length} units?`)) {
        setIsSubmitting(true);
        try {
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold font-headline text-pink-600 uppercase tracking-tight">{t('units')}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <KPICard title={t('totalUnits')} value={units?.length || 0} icon={Ruler} colorClass="bg-pink-600" />
        <KPICard title={t('active_status')} value={units?.length || 0} icon={Database} colorClass="bg-blue-600" />
      </div>
          ...
          <Table>
            <TableHeader className="bg-muted/10">
              <TableRow>
                ...
                <TableHead className="h-10 text-[10px] uppercase font-black text-center">{t('isDefault')}</TableHead>
                <TableHead className="text-right h-10 pr-6"></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered?.map((u) => (
                <TableRow key={u.id} className={cn("h-12 hover:bg-muted/5 transition-colors group", selectedIds.includes(u.id) && "bg-blue-50/30")}>
                  ...
                  <TableCell className="text-xs font-mono text-pink-600 font-black">{u.shortName}</TableCell>
                  <TableCell className="text-center">{u.isDefault ? <span className="text-[9px] font-black text-green-600 uppercase bg-green-50 px-2 py-0.5 rounded-full">Default</span> : "---"}</TableCell>
                  <TableCell className="text-right pr-6">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-32">
                        <DropdownMenuItem className="text-xs font-bold" onClick={() => { setSelectedRecord(u); setIsModalOpen(true); }}><Edit className="mr-2 h-3.5 w-3.5" /> {t('edit')}</DropdownMenuItem>
                        <DropdownMenuItem className="text-red-600 text-xs font-bold" onClick={() => deleteDoc(doc(db!, "companies", companyId!, "master_units", u.id))}><Trash2 className="mr-2 h-3.5 w-3.5" /> {t('delete')}</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl rounded-[2rem]">
          <DialogHeader className="bg-pink-600 p-6 text-white">
            <DialogTitle className="text-xl font-bold font-headline uppercase">{selectedRecord ? t('edit') : t('addUnit')}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSave} className="p-6 space-y-4 bg-slate-50">
            ...
            <div className="space-y-1"><Label className="text-[10px] font-bold uppercase">{t('shortName')}</Label><Input name="shortName" defaultValue={selectedRecord?.shortName} required className="h-10 text-xs rounded-xl" /></div>
