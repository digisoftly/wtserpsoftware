...
export default function MasterProductTypesPage() {
  ...
  const typesQuery = useMemoFirebase(() => {
    ...
    return query(collection(db, "companies", companyId, "master_product_types"), orderBy("name"));
  }, [db, companyId]);
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Duplicate Check
    const exists = types?.some(typ => 
      typ.name?.toLowerCase() === name.toLowerCase() && 
      typ.id !== selectedRecord?.id
    );
    ...
    try {
      const docRef = selectedRecord ? doc(db, "companies", companyId, "master_product_types", selectedRecord.id) : doc(collection(db, "companies", companyId, "master_product_types"));
      await setDoc(docRef, { ...typeData, createdAt: selectedRecord?.createdAt || serverTimestamp(), id: docRef.id }, { merge: true });
      ...
      if (confirm(`Delete ${selectedIds.length} items?`)) {
        ...
        try {
          ...
          selectedIds.forEach(id => {
            batch.delete(doc(db, "companies", companyId, "master_product_types", id));
          });
          await batch.commit();
          ...
          errorEmitter.emit('permission-error', new FirestorePermissionError({
            path: `companies/${companyId}/master_product_types/...`,
            operation: 'delete'
          }));
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold font-headline text-pink-600 uppercase tracking-tight">{t('productTypes')}</h1>
        <Button className="bg-pink-600 hover:bg-pink-700 gap-2 rounded-full h-9 px-6 text-[10px] uppercase font-bold shadow-lg" onClick={() => { setSelectedRecord(null); setIsModalOpen(true); }}>
          <Plus className="h-4 w-4" /> {t('addProductType')}
        </Button>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <KPICard title={t('productTypes')} value={types?.length || 0} icon={Box} colorClass="bg-pink-600" />
        <KPICard title={t('active_status')} value={types?.length || 0} icon={Box} colorClass="bg-blue-600" />
      </div>
                  ...
                  <TableCell className="text-right pr-6">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-32">
                        ...
                        <DropdownMenuItem className="text-red-600 text-xs font-bold" onClick={() => deleteDoc(doc(db!, "companies", companyId!, "master_product_types", typ.id))}><Trash2 className="mr-2 h-3.5 w-3.5" /> {t('delete')}</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl rounded-[2rem]">
          <DialogHeader className="bg-pink-600 p-6 text-white">
            <DialogTitle className="text-xl font-bold font-headline uppercase">{selectedRecord ? t('edit') : t('addProductType')}</DialogTitle>
          </DialogHeader>
