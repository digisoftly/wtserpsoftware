...
export default function MasterBrandsPage() {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await setDoc(docRef, { ...brandData, createdAt: selectedRecord?.createdAt || serverTimestamp() }, { merge: true });
      ...
      {isLoading ? (
        ...
        <Card className="border-none shadow-sm rounded-xl overflow-hidden bg-white ring-1 ring-slate-100">
          <Table>
            ...
            <TableBody>
              {filtered?.map((b) => (
                <TableRow key={b.id} className="h-12 hover:bg-muted/5 transition-colors">
